create function mapping.get_resource_weighted_capacity(p_resource_uids text[] DEFAULT NULL::text[], p_from date DEFAULT (CURRENT_DATE - 30), p_until date DEFAULT CURRENT_DATE, p_hours_per_day numeric DEFAULT 18, p_oee numeric DEFAULT 1, p_nesting_eff numeric DEFAULT 1) returns TABLE(resource_uid text, resource_name text, width integer, height integer, sides integer, material_id integer, material_name text, profile_name text, total_amount bigint, weight numeric, panels_hour numeric, panels_per_day numeric, capacity_sqm_per_day numeric, param_from date, param_until date, param_hours_per_day numeric, param_oee numeric, param_nesting_eff numeric)
	stable
	language sql
as $$
...
alter function mapping.get_resource_weighted_capacity(text[], date, date, numeric, numeric, numeric) owner to xfw3;
