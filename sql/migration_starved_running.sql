-- ============================================================
-- Migration: the derived residual of the shift builder is renamed
-- from 'data-error' to 'starved.running'.
--
-- running − producing is the machine waiting (for an operator), not a
-- data problem. It keeps its own code so producing + starved.running
-- = running stays verifiable per shift; the lookup gives it
-- alias_of = 'starved', so it displays and counts as starved. The
-- logged states 'starved' (Durst) and 'starved.operator' (Zünd) are
-- untouched.
--
-- Note: log.get_resource_state_aggregate (the v1 donut) still derives
-- its own 'data-error' on the fly from relation.lookup — that reader
-- is deliberately untouched until v1 moves over.
--
-- This supersedes the residual naming in sql/migration_state_shift_agg.sql
-- (already executed). Run order:
--   1. this file
--   2. sql/update_log_lookup_resource_state.sql (regenerated: the
--      lookup now carries the starved.running node)
--   3. sql/log/get_resource_state_shift_totals.sql (unchanged, run it
...
-- the rows the builder already wrote under the old name
UPDATE log.state_shift_agg
SET state = 'starved.running'
WHERE state = 'data-error';
...
-- 1. no data-error rows left; expected: 0
SELECT count(*) AS data_error_rows
FROM log.state_shift_agg
WHERE state = 'data-error';

-- 2. producing + starved.running must equal running exactly;
--    expected: no rows
WITH x AS (
    SELECT shift_date, shift_index, resource_uid,
           ...
           sum(duration_seconds) FILTER (WHERE state = 'starved.running') AS starved_running
    FROM log.state_shift_agg
    GROUP BY 1, 2, 3
)
SELECT * FROM x
WHERE running IS NOT NULL
  AND abs(coalesce(producing, 0) + coalesce(starved_running, 0) - running) > 1;
