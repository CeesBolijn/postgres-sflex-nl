...
-- weeks (and ahead) on their planned slots, with the same
...
LEFT JOIN LATERAL (
    -- latest slot at or before the nest moment, else the first of the day
...
SELECT count(*)                                          AS nests_in_window,
       count(*) FILTER (WHERE lane_item_id IS NOT NULL)  AS on_planned_slot,
       count(*) FILTER (WHERE lane_item_id IS NULL
                          AND lane_id IS NOT NULL)       AS on_created_slot,
