...
(
	...
	status text default 'draft'::text not null
		constraint formula_status_check
			check (status in ('draft', 'pending-approval', 'active', 'archived')),
...
comment on column formula.status is 'draft -> pending-approval -> active -> archived. At most one active version per code; archived versions stay valid for what was created in their time.';
